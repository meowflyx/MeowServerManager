...
- Установка модов с автоматическим разрешением зависимостей **всех уровней**
  (прямые + транзитивные — зависимости зависимостей тоже тянутся), с
  защитой от циклов и дедупликацией по проекту.
- Авто-определение стороны (client/server/both) по метаданным провайдера и
  маршрутизация каждого файла (включая зависимости) в подходящие папки.
...
- CurseForge не отдаёт чёткого client/server-разделения, поэтому CF-моды и их
  зависимости по умолчанию ставятся в обе папки (лишний клиентский мод
  безобиден, отсутствующий — ломает вход).
- Уже скачанные файлы с совпадающим хэшем (Modrinth) или размером (CurseForge)
  при повторной установке пропускаются — общие транзитивные зависимости не
  качаются повторно без нужды.
